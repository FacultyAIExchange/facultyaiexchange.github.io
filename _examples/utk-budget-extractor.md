...
summary: A Python tool that turns a folder of institutional budget spreadsheets into proposal-ready justification templates, LaTeX macros for every number, and one merged workbook with formulas intact.
...
Point this tool at the folder and it produces, for each input file, a set of LaTeX macro definitions covering every budget figure and an individual budget justification. Across all inputs it produces combined definitions, a unified justification, faculty summary tables in both one-row-per-person and year-by-year form, a master driver document that aggregates the justifications, and a merged Excel workbook. The merge writes only user-input cells and never overwrites a formula, so the consolidated workbook stays live rather than becoming a flattened snapshot.
...
This tool was originally hand written years ago after I caught a math error in a justification in a submitted proposal. Horrified knowing that it came from a forgotten copy-and-paste, I vowed to never have that happen again and built a latex builder such that no manual numbers needed to be typed. Claude helped expand this into multi-PI proposal handling, automatic skeleton justification writing and merging.
